...
Links and credits are left out. An event's `sources` list, an annotation's `wikipedia_url`, and an image's `creator` each yield a claim about a link or a byline that the biographical materials cannot settle either way; the sources travel in the unit's context, so an evaluator still sees what the event cites. A chapter's `illustration` block is left out for a different reason: it holds the prompt its artwork was generated from, and "the illustration shows luminous planes in darkness" is true of the picture by construction.
