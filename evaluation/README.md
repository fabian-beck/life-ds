...
Two things are deliberately left out. An event's `sources` list is provenance rather than assertion, and whether a listed URL resolves is already checked by `scripts/validate_source_links.py`; it travels in the unit's context, so an evaluator still sees what the event cites. A chapter's `illustration` block holds the prompt its artwork was generated from, and "the illustration shows luminous planes in darkness" is true of the picture by construction.
