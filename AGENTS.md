...
The report's PDF is built from LaTeX and in no other way. Every build writes the report to `docs/report/latex/report.tex`, including an appendix that lays out on paper what the step note shows on screen: `npm run report:figures` prints the drawn figures it includes from the built page, and `npm run report:latex` compiles it with Tectonic or any TeX distribution to `docs/report/latex/report.pdf`. The page itself has no print layout. See [Development reference](docs/agent/development-reference.md#the-latex-rendering).
...
Sessions notice defects that have little to do with their own task—a broken control on an adjacent screen, a script that fails on a case nobody asked about, a check that is red on `main`. Do not widen the session to fix them, and do not let them vanish when the session ends. Open a GitHub issue in `fabian-beck/life-ds` instead, using whatever GitHub access the session has, such as the `gh` CLI or the GitHub MCP tools.
...
- **It is clearly a bug.** Something crashes, produces a wrong result, or contradicts documented behavior. Refactoring ideas, style preferences, and improvement wishes are not bugs; keep them in the session summary unless the user asks for an issue.
- **It is a defect in the implementation, not in the data.** The subject of an issue is code, a script, a test, a locale file, the interface, or the documentation. Never file an issue about the content of a generated dataset under `data/`: a wrong sentence, a dead link in a network, a palette that fits no one, a dataset that predates a schema change. The datasets are corrected and regenerated continually, so such an issue documents a snapshot that no longer exists by the time anyone reads it, and it invites a hand repair where the no-repair rule wants a generator fix. A data finding goes in `data/outdated.md`—with the dataset, the generator change behind it, and what the reader loses—and is named in the session summary. Where the generator is what let the content through, the generator is the bug, and the issue says so in those terms: the step, the prompt or schema that permits it, and one dataset as the example.
...
Write the issue for someone who does not have this session's context: what happens, what should happen instead, how to reproduce it, and the file and line where it starts. Reproduce it from code and fixtures, and quote no dataset content as the evidence—the corpus under the issue will have moved on. Label it `bug`. Do not file an issue for something the session already fixed, and do not file one for a bug inside the session's own task—fix that.

A red check on `main` is an issue like any other, and the subject is the check: a test that asserts over the shipped corpus is itself the defect, because the datasets move and the assertion does not. Such a test is removed rather than re-fitted to the current data, and the rule it tried to hold is left to the generation step that writes the field—see [Testing strategy](docs/testing-strategy.md#what-not-to-add).
