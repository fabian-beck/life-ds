...
def build_prompt(subject: str, person_id: str, context: Dict[str, Any]) -> str:
    details: list[str] = [
        ...
        "- The pattern should be highly stylized and work as a tiled background.",
        "- The pattern must tile seamlessly: a mark that meets an edge continues at the opposite edge, at the same coordinate along it. A diagonal leaving the right edge at y=40 re-enters at the left edge at y=40, still rising at the same angle. A circle centered on the top-left corner is drawn three more times, on the other three corners, so that four quarters make one circle where four tiles meet. A motif an edge cuts is drawn a second time against the opposite edge, cut the other way.",
        "- Nothing may simply stop at an edge. The tile has no frame, and a stub that ends on one reads as a broken line once the tile repeats.",
        "- Seamlessness is a rule about the edges, not about the design. Keep the number of marks that touch an edge small and pair each of them deliberately; the interior may be as rich as the person warrants. Never thin a composition out to make the edges easier—a crossing is placed, not removed.",
        "- A lattice of horizontal and vertical lines is what this task falls into when nothing resists it, and four such tiles beside each other are indistinguishable. Draw one only where the person's own work is rectilinear. Otherwise carry at least one family of circles, arcs, or diagonals, which cross an edge as cleanly as a straight line does.",
        "- Strong strokes are favored over thin lines for better visual impact.",
        "- Create visual hierarchy through stroke-width variation (e.g., 2px, 4px, 8px) rather than color or opacity. The heavy weights carry the pattern and 1px is an accent; a tile drawn thin throughout reads as a faint grid rather than as this person's mark.",
        "",
        ...
        "- Vary density rather than intricacy: minimalists get sparse clean geometry, systematic thinkers a tighter weave, revolutionaries a bolder asymmetric rhythm—each still built from a few repeating families.",
