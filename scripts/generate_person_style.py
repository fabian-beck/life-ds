...
def build_prompt(subject: str, person_id: str, context: Dict[str, Any]) -> str:
    details: list[str] = [
        ...
        "- The pattern must tile seamlessly: a mark that meets an edge continues at the opposite edge, at the same coordinate along it. A stroke reaching x=160 at y=40 needs its continuation at x=0 at y=40, and one reaching y=0 at x=24 needs it at y=160 at x=24. A motif an edge cuts is drawn a second time against the opposite edge, cut the other way.",
        "- Nothing may simply stop at an edge. The tile has no frame, and a stub that ends on one reads as a broken line once the tile repeats. Marks crossing the whole tile, and families of parallel lines whose spacing divides 160, satisfy the rule most easily.",
        "- Keep the tile simple. A few families of marks on a regular grid are what makes the two rules above achievable and what survives being printed large behind a story; a crowded composition of one-off marks meets the edges in too many places to keep them all paired. Distinction comes from the choice of motif and its rhythm, not from the number of marks.",
