...
**Birth and Death Descriptions** — the two boundary events carry the slide's facts in their `event_class`, so their prose is held to a goal of its own rather than to a fixed shape. Read each against the goal below and rewrite via `new_description` where the reader would not reach it: a birth that only narrates the birth the slide already shows, a death that only restates the cause, and, above all, a slide whose card and prose cannot be read together — parents on the card whom the prose never connects to the people who raised the child, a cause the prose leaves unexplained. Write what the goal asks from what the Wikipedia sources say, in the shape that life needs:
