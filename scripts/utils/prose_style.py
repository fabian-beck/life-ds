...
def description_contract_prompt() -> str:
    ...
    return (
        ...
        "- It is read in sequence. The reader arrives from the slides before it and "
        "knows what those said and nothing else. A section, a machine, a method, an "
        "office, or a work the description leans on is named for what it is where the "
        "story first meets it, and what happened on an earlier slide is not told "
        "again. Naming this event's own place and people is not repetition; retelling "
        "the slide before it is. What the slide adds to the story is what the "
        "description says, and a slide that only restates the ones around it says "
        "what changed at this moment or it has no reason to exist.\n"
        ...
        "Yale.'\n"
        "BAD (leans on what no slide introduced, adds nothing to the slide before): "
        "'Turing leads Hut 8 at Bletchley Park. The section works on German naval "
        "Enigma messages.'\n"
        "GOOD: 'Turing takes charge of Hut 8, the Bletchley Park section that reads "
        "the German navy's Enigma traffic. The naval operators disguise their message "
        "settings with secret bigram tables, and the traffic stays unread while the "
        "army and air force keys are already being broken.'\n"
    )
