...
def render_pipeline(writer: Writer, mount: Mount) -> str:
    ...
    return (
        "\\begin{figure}[tbp]\n"
        "\\centering\n"
        "\\includegraphics[width=\\textwidth,height=0.86\\textheight,"
        f"keepaspectratio]{{{figure.file}}}\n"
...
def render_screenshot(writer: Writer, mount: Mount) -> str:
    ...
    if width and width <= MARGIN_FIGURE_MAX:
        size = f"width={width / CSS_PX_PER_MM:.1f}mm,height=0.55\\textheight,keepaspectratio"
