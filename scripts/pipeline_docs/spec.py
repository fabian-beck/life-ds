...
which phases exist, what order they run in, and which artifacts they read and
write. Everything factual about a phase — its model, reasoning effort, output
schema, prompt text, CLI flags — is pulled from the source by `introspect.py`
and must not be duplicated here.

Each step names a `script` and a `function`. `validate.py` checks that both
still exist and that no AI call site in the codebase is left unclaimed, so a
phase added to the pipeline without a spec entry fails the docs build instead
of quietly going undocumented.
"""
...
STEPS: List[Step] = [
    ...
    Step(
        ...
        stage=0,
        summary=(
            "Runs the six person steps in order and keeps going when the "
            "optional ones fail, so a missing portrait never costs a biography."
        ),
        outputs=[],
    ),
    Step(
    ...
    Step(
        ...
        summary=(
            ...
            "preserving the attribution each file requires."
        ),
    ...
    Step(
        ...
        stage=0,
        summary=(
            "Runs phases 1–8 over the existing person datasets. Every phase after "
            "the plan is non-fatal: a failure keeps the bottom-up text."
        ),
    ),
    Step(
        "m_p1",
    ...
    Step(
        ...
        "derive_clusters",
        stage=8,
        summary="Community detection over the reviewed graph produces the story's circles.",
    ),
    Step(
        "m_p6",
    ...
    Step(
        ...
        stage=10,
        phase_label="Phase 7",
        summary="Groups the story's located events geographically into candidate map stops.",
        skip_flag="--skip-map",
    ),
    Step(
        "m_p7b",
        ...
        stage=11,
        phase_label="Phase 7",
        summary="Scores how much each located event carries the theme, weighting the clusters.",
