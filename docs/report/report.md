...
disclaimer:
  This report was co-written with AI. Its text and figures were drafted and revised with language models under the authors' direction, along manual edits. The system the report describes was implemented by AI through agentic engineering.
abstract:
...
The [[meta-pipeline|meta pipeline]] traces a theme across many lives and consumes the output of the personal pipeline instead of external sources. The chart[[figure:pipeline-meta]] bands its steps into six phases: the story is planned and its events collected; the events are curated against the theme; the social network and the map are derived on two independent branches, each structured deterministically, reviewed or weighted in an inference pass, and narrated; composition joins the branches into one story and gives it a visual identity; and localization closes the run.
