...
As in VisKonnect, the operations this requires are interpretive and make it necessary to structure the unstructured textual biography. Selecting the episodes that constitute a life, identifying the places, and judging which relationships are relevant leverage [[inference|large language models (LLMs)]]. The system is accordingly built in two parts that [[artifacts|connect through the data]]. Generation runs offline as preprocessing, in which LLM-based generation is interleaved with deterministic transformation: a step transforms, researches, or reviews material, and the steps around it resolve, cluster, merge, and validate what the model returned. The processing produces a set of documents describing one life or one theme. The interface loads those documents and renders them, so that reading a story is then a matter of presentation alone, implemented through web technologies as a single page application.
...
At the core of the approach is the data that connects story generation and the interface, and it is the central abstraction of the system. For individual biographies, it provides a multi-faceted view—several structured perspectives on the original encyclopedic source: <<profile|the individual>> a story is about, the <<events|events>> that constitute the main turning points of the life, the <<narrative|text>> that describes them, the <<imagery|pictures>> that illustrate them, and the <<places|geography>> a life traces. Further facets are more interpretive, and are stated by no article as such: the <<network|social network>> of the respective ego, and the <<identity|visual identity>> in which the story is styled individually.

::: conceptlegend
:::

Referencing the individual lives' data, the meta story requires further data abstractions ((second-order)). Each meta story builds on a <<theme|theme>> that names the cast of lives it draws on and groups them into subtopics. Its chapters cut across the biographies into shared periods, and each of them holds <<events|events>> that remain addressed in the person's own data, annotated with what they contribute to the theme. The individual ego networks are merged into one <<network|social network>>, in which people who recur in several lives bridge subjects who never met directly, and the <<places|geography>> of all of those lives is clustered into the few places the theme turns on. The <<narrative|text>> is written for the theme rather than assembled from the lives it draws on, and the <<imagery|pictures>> that illustrate it are selected from the events of the individual stories. Like a life, a theme is presented in an <<identity|visual identity>> of its own ((own-look)).

## Generation

Generation runs offline as Python command-line scripts, one entry point per story type, invoked for a single subject at a time. Each step reads the documents earlier steps wrote and writes its own back to disk, so a run is resumable, a single concern can be regenerated without touching the rest, and a failing step is recorded and skipped rather than aborting the subject. What a run leaves behind is a directory of JSON documents, versioned alongside the application that reads them.

Each step in the pipeline figures below is an address rather than a label. Opening one gives the record behind it: the function and line that implement it, the model and reasoning effort it resolves, the output schema it fills, what it reads and writes, the steps it needs and feeds, and the prompts it sends. On paper the same records are laid out for every step as an appendix.
