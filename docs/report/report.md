...
The narrative is derived from that material in two calls of different types. The [[step:p_events_p1|first call]] proposes 12 to 16 events with titles, dates, descriptions, a classification, and a weight, and groups them into three to six chapters. It is the only call that sees the life as a whole, which weighting and grouping require. Its result is a skeleton without places, sources, or pictures; chapters are dated afterwards from the events they hold. A [[step:p_events_p2|second call per event]] researches it against the relevant articles: the historic and modern name of its place, the people involved, the sources, a semantic icon, and image searches that would illustrate it.

::: note
Researching events individually multiplies model calls by the number of events. Each call receives a focused prompt and a small output schema, a malformed response affects one event only, and the calls run in parallel.
:::

Places and pictures are then resolved against external registers. [[step:p_geocode|Geocoding]] places each location on a modern map, preferring the modern name from event research. Image search runs twice: [[step:p_img_search|searches for the life as a whole]] name the person and the subjects of the events, and the per-event searches name specific objects such as a building or a document. [[step:p_img_fetch|All queries]] run against Wikimedia Commons and Openverse, and the hits are deduplicated by URL. [[step:p_img_filter|A deterministic filter]] scores the candidates on resolution, file size, and filename before [[step:p_img_match|a matching call]] assigns pictures to events, writes captions with attribution, and picks a reference portrait; an event may stay without a picture. Since the matching call judges by metadata only, [[step:p_img_verify|a vision call]] checks whether the chosen portrait depicts the subject and drops it otherwise.

[[step:p_write|Writing the dataset]] persists the result; every later step reads the written documents, which makes a run resumable and a single concern regenerable. [[step:p_register|The registry]] adds the subject to the index of the landing page. Two descriptions of the whole life are then derived independently: [[step:p_style|a color and type system]] from the subject's era and field, and [[step:p_network|the ego network]] as typed, weighted, and described relationships the article implies. Generated illustrations come last: [[step:p_portrait|the reference portrait is style-transferred]] toward one master style shared by all subjects, [[step:p_chapter_concepts|each chapter is turned into an abstract visual concept]] in one call over all chapters, and [[step:p_chapter_art|each concept is drawn]] in the same style. Both drawing steps take their colors from the style and are skipped when it is missing.

[[step:p_review|A review pass]] over the events and the network applies only high-confidence changes. Reading the slides in order, it rewrites descriptions that rely on names not yet introduced or repeat the previous slide. The style needs no review: its rules on tile, font, and contrast are checked in code, and [[step:p_style|the generator]] asks the model again with the reason when a check fails. Localization closes the run in three steps. [[step:p_name_evidence|A deterministic pass]] collects the target language's names of the subject, the people, and the places from the linked articles. [[step:p_glossary|A glossary call]] fixes once per subject how every name is rendered, since the interface matches cross-references on exact names. [[step:p_translate|The translation]] translates the extracted fields and merges them over a copy of the English document, so that dates, coordinates, URLs, and identifiers cannot drift.

[[step:p_backgrounds|A background report]] of a few hundred words is written after the review for roughly one event per chapter, selected by the interface's own rule and grounded in the reviewed text. [[step:p_illustrations|A critic call]] reads the illustrations found for it against the report, since keyword search returns much that merely shares a word with it. Datasets that predate a model field are not repaired but flagged as outdated and regenerated.
...
The generation side uses {{ pipeline.models }}. The model is resolved per [[inference|call site]], so that high-volume and quality-critical phases need not share one setting. Composition, the one step that reads a whole assembled story, takes the largest model; the portrait step takes the image model; the remaining steps are divided between a reasoning tier and a small one by whether a wrong answer can quietly become part of the corpus. A step uses the small model when its output is validated against existing entities, rewritten by a later step, or backed by a deterministic fallback: article selection, event research, image matching, event curation, narration, and translation. Proposing the events of a life, selecting the people of a theme, reviewing another step's output, and the glossary use the reasoning tier. The division reflects verifiability, not difficulty.

Reasoning effort is configured per call site as well. Proposing the events of a life is an inference problem and receives a reasoning budget; writing search strings is slot filling and receives none; the threshold and selection decisions of the small model receive a small budget. Model and effort are chosen independently.

Both settings are arguments of one function through which every schema-filling step reaches the API and which returns the parsed object or nothing. Earlier, six steps used a second API without an effort parameter and ran at the default while the configuration claimed a per-step setting. The shared entry point also states the retry policy once: connection errors and rate limits are retried, malformed requests and refusals are not. What a failure means stays with the step: article selection falls back to the first candidates, an optional section is skipped, a stale translation is left for the parity check to report, and curation stops the run.

Prompts are constructed in Python functions, since they require conditionals and injected data. Every step that writes text a reader sees appends a shared block that names the habits of model prose, such as a contrast against an alternative nobody proposed or a closing sentence on what an event "marked", and gives the plain sentence to write instead; a generic instruction to be concise changed nothing. Structured output is declared as Pydantic models, so the shape of a response is a type and validation precedes use. Model output is applied deterministically: identifiers are matched against existing entities, unknown references are discarded with a warning, and coordinates, URLs, and graph structure are copied, so that a defective response stays confined to its field.
