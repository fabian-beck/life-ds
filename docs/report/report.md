...
A last group of steps runs for no subject during generation and for one subject at a time afterwards. When a field is added to the model—[[step:p_classes|an event's classification]], [[step:p_weights|its weight]], [[step:p_backgrounds|its background report]]—every dataset written before it carries none, and regenerating those lives would rewrite reviewed prose in order to obtain an addition. Each backfill therefore reads the stored events and adds the one thing that is missing, against the prompt or the rubric of the phase that would have written it, imported rather than restated so that the two cannot drift. A backfilled background report is illustrated with one call more than an event is: [[step:p_illustrations|a critic reads what the searches returned against the report itself]], because half of what a keyword search returns merely shares a word with it.
