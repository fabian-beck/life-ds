...
A meta story presents a theme across several biographies, and its structure is the one the composition step imposes: an opening scene, three component sections, a conclusion, and a closing grid of the people the story is built from. Each of the three middle sections pairs running prose with one interactive component—a timeline, a network, a map—and each carries the article layer's context rather than a recapitulation of what the component already shows. Their order is part of the composition rather than a property of the reader: the three answer different questions of a theme, and the composition step decides which of them a given theme is best entered through, writing each section's prose for the position it will occupy.
