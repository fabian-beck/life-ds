...
The generation process is split into stages. The first stage reads an encyclopedia article and writes the data of an individual biography; a second reads the individuals data and writes a meta story connecting multiple individuals. Both pipelines run offline as Python command-line scripts, one entry point per story type, invoked for one life or one theme at a time. Each step reads the documents earlier steps wrote and writes its own back to disk, so a run is resumable, a single concern can be regenerated without touching the rest, and a failing step is recorded and skipped rather than aborting the run. A run produces the directory of JSON documents described in the previous section. Each step belongs to one of [[kinds|four kinds]], which distinguish how steps can fail, what they cost to rerun, whether they depend on external services, and how their outputs can be checked. Only steps that require judgment are nondeterministic. Clustering, geocoding, chapter fitting, network merging, and translation bookkeeping remain deterministic.
