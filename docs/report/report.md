...
The sequence uses one axis, which leaves the other free to mean something else. On a life's landmarks the slide carries a second screen beneath the fold, so that sideways is *later* and downward is *further into the same event*. The layer below holds what the slide keeps a tap away or omits: the place under both the name it had and the name it has, the terms the description leans on, the images with their captions and credits, the persons with their relationship to the subject described, and the sources for that one event. What leads it is written rather than assembled. Phase 2 returns, alongside the places and people and sources it extracts, a short passage of background addressed to a reader who has just finished the event: what surrounded it, why it mattered then, what followed from it. The prompt's operative rules are negative—the description is given as the thing to go beyond, and repeating any of it counts as a failure—because the failure mode of a summarizer pointed at an event it has already been shown is to say it again in other words. What the reader has is spelled out for it rather than left implicit: the terms the slide already explains behind a tap, quoted in full, the persons its chips already introduce, the events on either side that a swipe away reaches, and the subject's summary as assumed knowledge. The annotations matter most of that list, because their popups open directly above where the passage will be read, and a sentence that explains one of them again is repetition the reader cannot miss. This is the only step in either pipeline asked to write for a reader instead of to fill a schema, and it is the reason the phase is back on the larger model: every other field it returns is checked afterwards against a catalog, a geocoder, or a set of known entities, and a paragraph of prose is checked by nobody.

Under that passage the remaining records are set as prose too, rather than as a card of labeled fields: each is written out as a sentence and the sentences are run together into paragraphs. Neither the annotated terms nor the persons are among them, for the same reason the written passage is steered around both—each already has a way of being opened on the slide above, and a layer that reprints them is a second copy of it. What remains is the place under both of its names, which nothing else states. Which sentence a record becomes depends on how the generator left it—an explanation that already names its subject is left to stand, one that opens with an article becomes a statement about that subject, and the rest are kept as appositions, since what opens them is a proper noun that cannot be lowercased. Which events offer it is decided from a weight derived per event—its classification or the kind of event its icon names, its annotations, images, persons, sources, and the length of its description—taken one chapter at a time, so that each chapter offers its heaviest event and no more and the deep slides fall across the story rather than clustering where a life happens to be best documented. The offer is announced by a chevron in the space the slide keeps clear, and the map, which belongs to the event, fades out as the reader descends past it.
