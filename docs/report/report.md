...
The landing page stages the corpus three ways: as [[landing.carousel|a carousel of the curated collections]], as [[landing.filters|filters over the roles the profiles record]], and as [[landing.grid|a grid of portrait cards]], one per subject.
...
@carousel 476,99,754,397 Collection carousel
The curated collections as a paged carousel; each card is a meta story, entered through its own opening scene.
@filters 316,540,850,66 Role filters
One chip per role the profiles record, with the count of subjects holding it; selected chips narrow the grid below.
@grid 48,700,1182,120 Portrait grid
One card per subject—the generated portrait, the name, and the lifespan—so the whole corpus is browsable from one screen.
:::
...
@axis 0,54,1280,86 Year axis
The years the theme spans, on one scale every lane shares; contextual events—a war, a prosecution, an ouster—are marked above it.
@lane 392,404,364,48 A subject's lane
One life as a lane: the portrait and lifespan at its head, and the theme's events marked along it in the subject's accent color.
:::
