...
The sequence uses one axis, which leaves the other free to mean something else. On a life's landmarks the slide carries a second screen beneath the fold, so that sideways is *later* and downward is *further into the same event*. The layer below holds what the slide keeps a tap away or omits: the place under both the name it had and the name it has, the terms the description leans on, the images with their captions and credits, the persons with their relationship to the subject described, and the sources for that one event. It is set as a passage of prose rather than as a card of labeled fields, because the material is a continuation of the reading rather than an apparatus attached to it: each record is written out as a sentence and the sentences are run together into paragraphs, so that the terms an event leans on read as a short chapter of background rather than as a glossary. Which sentence a record becomes depends on how the generator left it—an explanation that already names its subject is left to stand, one that opens with an article becomes a statement about that subject, and the rest are kept as appositions, since what opens them is a proper noun that cannot be lowercased. The wording is the dataset's own throughout; nothing on the reading side writes prose. Which events offer it is decided from a weight derived per event—its classification or the kind of event its icon names, its annotations, images, persons, sources, and the length of its description—taken one chapter at a time, so that each chapter offers its heaviest event and no more and the deep slides fall across the story rather than clustering where a life happens to be best documented. The offer is announced by a chevron in the space the slide keeps clear, and the map, which belongs to the event, fades out as the reader descends past it.
