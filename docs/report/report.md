...
The system is a working prototype rather than a study: two generation pipelines, an application that reads what they produce, and a corpus generated end to end by both, in the languages the corpus is published in. Its contribution is a division of labor between interpretation and presentation, and the four principles set out at the beginning are the claims that division makes. None of them failed, and none of them holds without qualification; what follows states for each what it delivered and where it did not, and then the issues that are independent of all four.

One record with several encodings ((one-record)) is the principle that holds most nearly as stated, because it holds by construction rather than by reconciliation: an event carries its date, its place, the persons in it, its sources, and its images, and the prose, the timeline, and the map are renderings of that one record, so they cannot disagree about when something happened or where. Two qualifications are due. The encodings do not cover the same material—only events whose places resolved to coordinates reach the map, which is therefore a subset of the timeline rather than a second view of it—and the social network is not the same record at all: it is generated as a separate artifact with the finished events as context, and merged across subjects by normalized name, so its agreement with the events rests on inference and string matching rather than on construction. Consistency is also not correctness. Encodings that share a record share its errors, and no step of either pipeline is positioned to notice.

Deriving bottom-up and revising top-down ((bottom-up)) addressed the problem it was introduced for, and the meta story is where it can be seen working: steps that observe only their own slice write locally sound and globally repetitive text, and the composition step, which reads the assembled document as a reader meets it, is what separates the captions bound to a component from the article prose surrounding it. The pattern is realized unevenly. In the meta pipeline the top-down pass is a full rewrite in one voice that may also drop subjects; in the personal pipeline it is a critic that applies only high-confidence corrections, so a life is revised in place rather than reconsidered as a whole, and the register problem the principle names is addressed there mainly by keeping the branches apart. The top-down step is also the least verifiable one in either graph. It takes a whole document as input and its effects are distributed across the output, so nothing asserts that a rewrite improved the story; only reading it does.

Themes as second-order artifacts ((second-order)) proved a productive constraint. Because the meta pipeline consumes finished biographies and no source of its own, a theme's timeline, map, and network are assembled from records that already exist and have already been reviewed, and the corpus becomes a graph in which a theme is navigable into its subjects and a subject back into its themes. The constraint's cost is that a theme cannot exceed the corpus: its planning step names the people that are missing and would strengthen it, which is useful precisely because it cannot act on them—adding one is a personal run, not a meta one. The merge is the fragile part. Ego networks are combined by normalized name, so a person named differently in two documents becomes two nodes, and the glossary that fixes names once per subject and language exists for the same reason on the translation side.

Every story looking like itself ((own-look)) is fully realized as a mechanism and least examined in its consequences. Palette, typography, background pattern, and, for a theme, the glyph and rule that punctuate its prose are generated per story and injected as custom properties, so the design system is data and the application holds no story's appearance. What the mechanism does not do is constrain the result: legibility is requested in a prompt and reviewed by a further model call rather than computed, no step measures a contrast ratio, and the interface compensates with a reader-side high-contrast mode. A generated identity also gives up what a fixed one provides, since a reader moving between stories cannot assume that a color or a typeface means the same thing twice.

Three open issues are independent of the principles. First, the system has not been evaluated with readers. Its checks are a small deterministic suite and model-led exploratory testing, which establish that the application works rather than that the stories are read as intended; whether the reader-driven affordances are taken up—whether the timeline is used as a control, whether a theme is followed into the biographies beneath it—is exactly what the spectrum the introduction places this work on would need measured [@segel2010narrative]. Second, factual verification is left to the reader. Every event carries the sources it was researched from and a critic pass revisits the generated data, but nothing establishes that a date, a place, or a relationship is right, and generated prose reads with the same assurance whether the claim behind it is well sourced or merely plausible. Third, the corpus inherits its sources: a life thinly covered in the encyclopedia is thinly told here, and the selection biases of that coverage pass unchanged into every encoding derived from it.

What the work supports, then, is a limited claim: encyclopedic prose carries enough structure that it can be converted without per-subject authoring into a document that is both readable as a narrative and inspectable as data, and the conversion is reproducible to the extent that its interpretive steps are confined, schema-constrained, and stored. It supports no claim yet about how such documents are read, or about their accuracy beyond what the recorded sources let a reader check. Those are the two places where the work needs evidence rather than further machinery.
