...
**A margin figure stays in view.** Beside the text, a figure is held a little under the viewport's edge from the moment its place in the document scrolls up to that line, and stays until the next margin figure takes over, or a wide block or a section heading pushes it off. The widget is an anchor without height in the flow, not a float, since a right float may not begin above the bottom of the one before it; the band holding the figure is positioned from the anchor into the margin, the figure is a sticky body inside the band, and `bindMarginFigures` in `assets/app.js` gives the band the height of the stretch the figure serves, moves a wide block that would start inside a figure's own height below it, and pads the report by whatever the last figure hangs past its end. A next figure declared clear of the one before rises over it and the covered one fades; one declared closer than a figure's height overlaps it at rest, so the two trade places by a crossfade as the later one's place reaches the reading line. A figure taller than the viewport is held by the edge the reader is moving toward: reading down, it stops once its bottom is in view; reading up, once its top is. Print and narrow layouts release both the band and the body.
