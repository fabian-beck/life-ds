...
Entries print in **IEEE** form—initialed author block, title in quotation marks,
italic journal or proceedings, `vol.`, `no.`, `pp.`, year, and the DOI. **Every
author is named**; nothing is cut to `et al.`, which is the one place this
departs from the style. Write the `.bib` with full given names—the formatter
reduces them to initials—and commit a compound surname with a comma
(`Henry Riche, Nathalie`), since that is the only place it can be recorded.
