...
**A margin figure stays in view.** Beside the text, a figure is held a little under the viewport's edge from the moment its place in the document scrolls up to that line, and stays until the next margin figure rises over it and it fades underneath, or a wide block or a section heading pushes it off. The float is the figure's place and its placeholder; the figure itself is a sticky body inside it, and `bindMarginFigures` in `assets/app.js` gives the float the height of the stretch the figure serves, pulling the margin box back to the figure's own height so nothing else on the page moves. A figure taller than the viewport is held by the edge the reader is moving toward: reading down, it stops once its bottom is in view; reading up, once its top is. Print and narrow layouts release both the band and the body.
