...
Published by **GitHub Pages** from the `deploy` branch: a push onto it runs `.github/workflows/deploy-pages.yml`, which builds the tree it carries and publishes it at https://fabian-beck.github.io/life-ds/. Pushing or merging to `main` does not publish; advancing `deploy` does, and that push is the only step (`git push origin origin/main:deploy`). `checks.yml` validates and releases nothing. Do not deploy unless the user asks for it.
...
`socialCardPlugin` substitutes `%SITE_URL%` in `index.html`, which the link-preview tags there use for `og:url`, `og:image`, and the canonical link. An unfurler resolves those against nothing, so they have to be absolute; the default in `vite.config.js` is the Pages address, and a deployment at another address sets `VITE_SITE_URL` in its own environment. Because a static host serves one document for every route and no unfurler runs the router, the tags describe the site rather than the story behind the hash—per-story cards would need one static file per story, emitted at build time.
...
The user-evaluation deployment stays on Netlify, which is why `netlify.toml` and `netlify/` are still here: its log endpoint is a function and its store is a Netlify service, and a static host provides neither. It is the same site in Vite's `evaluation` mode, built from the `deploy-evaluation` branch, with interaction logging and an analysis page, and is described in [User evaluation](user-evaluation.md). The Netlify site's production branch still watches `deploy` until it is disconnected in the dashboard; while it does, one push publishes to both hosts.
...
- **Keep MapLibre out of the initial bundle.** MapLibre, pmtiles, and `@protomaps/basemaps` total ~1.1 MB. `LandingMap.svelte`, `StoryMap.svelte`, and `MetaStoryMap.svelte` are the only modules that may import them, and all are themselves imported dynamically (`{#await import("./StoryMap.svelte")}`). A static import of either component from anywhere pulls all of it back into the entry chunk. Each map component must import `maplibre-gl.css` itself rather than relying on the other having been loaded. `src/utils/basemap.js` hands MapLibre its worker's address through `setWorkerUrl`, from a `?worker&url` import of the worker module: MapLibre 6 locates the worker through `import.meta.url`, which bundling does not preserve, and without the call no map loads a tile in development or in the production build.
- PMTiles uses HTTP range requests (efficient tile loading)
- **The map draws entirely from `public/`.** Tiles come from `basemap.pmtiles`, and the sprite and glyph assets the style needs are vendored in `public/basemap-assets/` by `npm run basemap:assets`, so a map view contacts no third party. The script derives which glyph ranges to fetch from the tiles themselves—every `name…` property of every feature—so replacing `basemap.pmtiles` with a different extract means rerunning it; `node scripts/vendor_basemap_assets.mjs --check` reports what is missing. `tests/test_basemap_assets.py` guards the tree and the style's URLs, because a gap only shows as a warning and a fallback letterform.
...
- The `virtual:mdi-icon-map` plugin (`vite.config.js`) builds the runtime lookup from the union of `icon_categories.py` and the icons actually present in `data/people`, since generated data drifts from the vocabulary. It validates every name against real `@mdi/js` exports and **warns at build time** about any that do not exist—that warning marks a dataset generated before `normalize_icon()` existed: flag it in `data/outdated.md` and regenerate it.

When a script writes to `data/people/**/*.json`: the files are prettier-ignored and formatted as they were generated, so never rewrite them with `json.dump` for a small change—it would reflow every line of all of them to change a handful of strings. Open them with `newline=""` when writing, or Python turns each `\n` into `\r\n` on Windows and does the same damage by a different route.
...
The page is styled as an academic report: black on white, square corners, and color reserved for the step kind alone. Both pipelines are drawn, as sections 4.1 and 4.2 rather than as tabs, each with its own filters, search, and selection; the step note is shared between them.

**A step entry stays at the level the report argues at.** The step note, and the appendix built from the same record, hold a description of two or three sentences and a five-row record: what the step reads and what it leaves behind, each as a phrase the summarizer wrote from the source in the vocabulary of the figures, and the model, reasoning effort, and calls per run measured from the code. Nothing else about a step is printed there. Its file and line, its output schema, the flag that skips it, and the literal prompt are the source, which the figures index rather than copy. A step's kind, pipeline, group, and phase are already drawn, so the record does not repeat them.

The chart is a **layered DAG, not a sequence**. An arrow means one step consumes what another produced (`Step.depends_on` in `spec.py`, with a label for the data that travels along it); a step's layer is the longest such chain reaching it, so steps drawn side by side are genuinely independent—the meta story's map branch and network branch really do run without seeing each other. The artifacts a step reads and writes are not drawn; the step note and the appendix list them, each behind its concept's glyph. Orchestrator `main()` functions are deliberately not steps: they impose an order without creating a dependency, and drawing them made a fork look like a chain.
...
**The main text carries no usage instructions for the report page.** The prose argues about the system, not about the page presenting it, so a sentence telling the reader how to operate the report—that a step reference can be pressed, that a figure opens a note on selection—is cut from the running text. That guidance lives in the caption under the figure it concerns (see the caption rule below) and in the page's own hint lines in `assets/app.js`. Describing what a figure shows remains prose's job; describing what happens when it is touched does not.

**The register is academic.** The prose is judged by how much of it is load-bearing. Every sentence advances the argument, so one that restates its predecessor, announces its successor, or asserts only that the system exists at some size is cut rather than improved, and what a specialist would want but the sentence cannot hold goes into a note. Precision comes before elegance. A concept keeps one term throughout, repeated rather than varied for its own sake; a claim is stated exactly as far as it holds, neither hedged into vagueness nor inflated past what the system does; and a quantity is named or measured rather than gestured at, which is why "a handful of scripts" says less than either "two entry points" or nothing at all. Name the thing that acts, prefer the active verb, and choose the plain word over the impressive one. A contrast earns its place only when the alternative is one the system could have taken or a reader would otherwise assume, so "X rather than Y" is worth writing for "researched individually rather than in a single call" and worth cutting for "a generation concern rather than an interface one" or "computable rather than assumed", where the second half exists only to be denied and the sentence says more without it. The same test applies to fronting. "What the architecture cannot establish is that the content is true" delays the subject and the verb to announce that something is about to be said; "The architecture cannot establish that the content is true" says it. Fronting is worth one use in a section, where the emphasis it moves to the front is the point being argued, and it is worth none as a default sentence shape. Sentence shape follows the argument, so any construction that becomes a paragraph's default rhythm—the colon before an elaboration as readily as the em dash—has stopped carrying meaning and started carrying style.
...
**Width is opt-in.** The report body is one measure of running text with a margin beside it, and nothing that is read—paragraph, list, legend, caption—is ever set wider than that measure. A `ComponentSpec` declares which of three the block is: `"measure"` (the default: a legend or a schema list is text in a table's clothing), `"wide"` for the drawings and tables that have something to do with the room, or `"figure"` when the width is a property of the material rather than of the kind, in which case `app.js` classifies the block as it mounts it. A figure narrower than the measure—a phone capture at 390 pixels—is shown at four fifths of its width, so that it does not stand on the page as large as a phone, and moves into the margin beside the text wherever the page is wide enough to hold both, and stays in the text column where it is not. The page reads text first and figure second: declare a screenshot directly below the paragraph that first names it. In the margin the figure is anchored to that place and may run ahead of it by its own height, so it stands beside the paragraph it follows.

**A margin figure stays in view.** Beside the text, a figure runs a little ahead of its paragraph: it may stand as much as its own height above its place in the document, but not over the wide block or section heading before it, so that it is in view, as completely as the viewport allows, from the moment its paragraph is. A figure that fits is held whole, by its bottom edge while its place is low in the viewport and by its top edge once its place has passed the pin line; one taller than the viewport is held by its top edge until its place passes the pin line, scrolls with the text as it does, and is held by its bottom edge from there on. It stays until the next margin figure takes over, which happens once the reader is done with its paragraph and the next figure actually covers it, or until a wide block or a section heading pushes it off. The takeover is a decision made on scroll, with a little slack before it reverses, and the crossfade runs for a fixed time rather than with the scroll, so a reader who stops is never left between two figures. The widget is an anchor without height in the flow, not a float, since a right float may not begin above the bottom of the one before it; the band holding the figure is positioned from the anchor into the margin, the figure is a sticky body inside the band with one inset for both edges, and `bindMarginFigures` in `assets/app.js` measures the band, moves a wide block that would start inside a figure's own height below it, and pads the report by whatever the last figure hangs past its end. Print and narrow layouts release both the band and the body.

**Captions complement the figure, they do not describe it.** A caption names what the block is and then adds only what a reader cannot get from the drawing itself: what an unlabelled mark means and what the block does when it is touched. Anything a legend, an axis, a node label, or the prose above already states is left out—the pipeline figure does not re-explain the layer semantics of section 4. Captions are written as sentences; the terminating full stop is added by `figureCaption()` in `assets/app.js`, so call sites need not repeat it.
