...
- `OPENAI_MODEL` — text/reasoning model for the steps that decide what a life or a theme is, read a whole document, or criticize another step's output (default: `gpt-5.6-terra`)
...
A step runs on `OPENAI_BULK_MODEL` when a wrong answer cannot quietly become part of the corpus — its output is validated against existing entities afterwards, rewritten by a later step, or backed by a deterministic fallback. That covers related-article selection, the research event research, image search and matching, both style generators, meta-story event curation, circle narration, and the map branch. Everything else keeps `OPENAI_MODEL`: the proposal, the ego network, story planning, historical context, the depth-layer background reports and their illustration critic, both review passes, the name glossary, whose decisions every other document then matches on by exact name, and the translation calls themselves, whose wording nothing downstream checks or rewrites. The report's step note shows the model and effort each call site actually resolves to; it is generated from the source, so consult it rather than this list when they disagree.
...
**Making the call** (see `scripts/utils/model_calls.py`): every step that fills a schema calls `parse_structured()`, which wraps the Responses API. Reasoning effort is a required argument there, so no step can silently inherit the model's default. The call retries only failures a second identical request could survive — dropped connections, timeouts, 408/409/429/5xx, and a response that parsed to nothing — and returns `None` for everything else, having logged the step's name and the reason. What a `None` means belongs to the step: article selection falls back to the first N candidates, historical context is skipped, translation is left stale for `--check` to report, and curation stops the run rather than let a story keep every event of every life.

**Running the independent calls side by side** (see `scripts/utils/concurrency.py`): the research of the events, the image searches, the chapter illustrations, and the background reports read nothing of each other, so each of those steps sends its calls through a small thread pool and gathers the results in the original order. `LIFE_DS_WORKERS` sets the pool size (default 4); `1` restores the serial order call for call and is the first thing to try when the provider answers 429, since every the research call carries the subject's article and a handful of them at once reach the tokens-per-minute limit. The geocoder stays serial because Nominatim allows one request a second, and the steps that read each other's output stay in sequence. The usage ledger counts calls from every worker; a `usage.step` block belongs on the main thread, because it attributes every worker's calls while it is open.
...
The story offers a depth layer on roughly one event per chapter, and the layer is a written background report plus the pictures that illustrate it. This step computes the story's own deep-event selection (`scripts/utils/event_depth.py`, a port of `src/utils/story/eventDepth.js` — keep the two in sync) and writes a report of up to 350 words for the events the story will offer one on. A selected event is where a report may go rather than where one has to: the call notes what its sources say about that event in particular before it writes, and an event they record without documenting gets none — the story then offers no way down there, and a rewrite that refuses drops the report and pictures the event was carrying. It re-decides the event's citations in the same call, report or none, and illustrates the report from Commons through a critic on the default model. Inside `generate_person.py` it runs after review and before translation; standalone it fills the same events for the persons named. A dataset whose events carry no `weight` predates proposal weighting — the step refuses it and points to `data/outdated.md`.
...
The result is stored under the chapter as `illustration` in `data/people/{person_id}/life_events.json`, synced to every translated copy (a path and a concept are not prose), and the WebP files are written to `public/chapter_art/{person_id}/`. The files are named by chapter id, so a regenerated story whose chapters were renamed would leave the old chapters' pictures behind; the step therefore ends by removing every file in that directory that no chapter of any language copy refers to. A dry run and a concepts-only run write no images and remove none. The concept is stored with it so the picture can be redrawn without paying for the text call again. No PNG master is kept — a corpus-wide set of them would outweigh every other asset in the repository.
...
### Event Proposal and Research

The life events generation uses **a proposal call and a research call per event** for improved accuracy and richer metadata:

**Proposal: event skeletons and chapters** (1 AI call)
- Identifies 12-16 significant life events (strictly enforced)
- Groups them into 3-6 coherent chapters by naming the chapter on each event, and writes the conclusion; the chapters are dated afterwards from the events they hold, and `validate_chapter_partition` refuses a plan whose chapters do not form contiguous runs of at least two events (`MIN_CHAPTER_EVENTS`) before the research pays for any research. A refused plan is asked for once more with the reason (`PROPOSAL_ATTEMPTS`); a second refusal fails the run
- Creates crisp titles (2-6 words) and writes each description to the shared description contract (`scripts/utils/prose_style.py`), the one definition the research and the review hold it to as well: 2-4 sentences that name the people and the place and stand without the card beside them
- Uses ALL related articles for broad context

**Research: the details of each event** (12-16 AI calls, one per event)
- Researches specific details for each event individually, and rewrites a skeleton that falls short of the description contract from the 30,000 characters of the subject's article it holds, where the proposal saw 12,000; a one-sentence skeleton is extended, never returned as it is
