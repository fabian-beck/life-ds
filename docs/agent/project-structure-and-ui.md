...
- `EventDepth.svelte` renders the passage: a sentence placing the event under the name it happened under and the name a map carries today, one figure with its caption and credit, a paragraph per term the description leans on, a paragraph per person, and a quiet line of sources. All of it is already in the dataset — the fold keeps it a tap away at most, and the place and the sources are not on the slide at all. The people move out of the fold on a deep slide (`peopleInDepth` on `EventSlide`) so the same names are not listed twice.
  ...
  What leads the layer is not composed at all. Phase 2 writes a `background` passage per event — the situation it sat in, why it mattered, what followed — and `EventDepth` renders it above the composed sentences. The prompt's operative rule is negative: the description is handed over as the thing to go beyond, and repeating it counts as a failure, because a summarizer pointed at an event it has just been shown will otherwise say it again in other words. `scripts/backfill_event_backgrounds.py` fills the field for existing datasets, sending Phase 2's own prompt rather than a copy of it, and takes a `--selection` file so only the events that actually open a depth layer cost a call. An event without a passage falls back to the composed sentences alone.
