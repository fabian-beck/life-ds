...
- `EventDepth.svelte` renders the layer: the written background report, its illustrations dealt out between the paragraphs with their captions and credits, and a quiet line of sources. Nothing that has an affordance of its own is repeated here. The annotated terms are marked in the description, where a tap opens the same explanation and the same link. The people are chips on the slide, and the chip is how this application gives a person's context everywhere — the layer briefly took the people for itself, which made the one slide with more to say the one slide where a person could not be opened.
  ...
  The illustrations are searched and then read. A Commons keyword search is a keyword search: "On Computable Numbers manuscript" returned a 16th-century Mexican codex, "Christopher Morcom" returned a steam engine built by Belliss & Morcom, and "Banbury sheets" returned an ordnance map of the town of Banbury. So `_fetch_background_images` shows every candidate to a critic on the larger model — filename as well as caption, because a Commons caption is often the uploader's paperwork — which keeps at most three, each of a different thing, and is told that three is a ceiling and not a target. One picture per query is enforced in code. `--images-only` redoes the illustrations for reports already on disk, reading fresh searches off the stored text: a report is good or thin, a search is right or a steam engine, and tuning the critic by rewriting every report is both expensive and a way of never seeing whether the critic improved.
