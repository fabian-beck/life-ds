...
- `EventDepth.svelte` renders the passage: a sentence placing the event under the name it happened under and the name a map carries today, one figure with its caption and credit, a paragraph per term the description leans on, a paragraph per person, and a quiet line of sources. All of it is already in the dataset — the fold keeps it a tap away at most, and the place and the sources are not on the slide at all. The people move out of the fold on a deep slide (`peopleInDepth` on `EventSlide`) so the same names are not listed twice.

  The register is prose, deliberately: no card, no rules, no icons, no section headings, a narrower measure than the slide's, and what a paragraph is about run into its first sentence rather than set above it as a label. It is meant to read as the story going on in a quieter voice. Genuinely new narrative — background an LLM wrote rather than material rearranged — would need a generation step and a new field on the event; nothing here invents prose.
...
The invitation down is `.depth-affordance`, a small chip held to the right edge in the gap the slide already keeps clear of the timeline, with two chevrons that beckon (and hold still under `prefers-reduced-motion`). It is on the right and not centered because centered it stood in the middle of the map. It is written in the flow after the event and lifted into the corner by CSS; on a slide marked `fold-overrun` there is no corner to lift it into, so it stays in the flow, after the event's last line. It fades as the reader descends. The story map fades with it: `StoryView` tracks the active slide's scroll as `depthProgress` and passes it to `StoryMap`, which drives `--map-depth-opacity` — the map belongs to the event, not to the page of running text that replaces it. Down and up arrows drive the second axis on a deep slide before carrying the reader on to the next one, and a wheel flick that opens the depth layer is not allowed to continue into the next slide in the same gesture.
