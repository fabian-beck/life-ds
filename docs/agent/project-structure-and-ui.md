...
│   ├── backfill_event_classes.py    # Give unclassified events the event_class block Phase 1 never wrote
│   ├── backfill_event_weights.py    # Weigh a life's events against each other, one call per person
│   ├── backfill_event_backgrounds.py # Background report per landmark event, its citations and its illustrations
...
`.slide` carries `content-visibility: auto`, which lets the browser skip the slides that are not on screen. Nothing about the story changes: a slide is sized by the strip around it, a fixed share of its width and a full screen tall, so skipping what is inside one moves nothing, and `inert` still decides what the keyboard and assistive technology can reach. What does change is that a skipped slide has no laid out content, so anything that measures one reads zeroes and concludes that everything fits. `watchContentFit` returns early for a slide the browser has skipped, and is called again when the slide is rendered, which is the first moment its answer is worth anything. `tests/interface.spec.js` walks the story slide by slide for the same reason, rather than reading every slide off in one pass — on screen is the only state a reader ever meets a slide in.
...
- `EventDepth.svelte` renders the layer: the written background report under its own section headings, its illustrations dealt out between the paragraphs with their captions and credits, and a quiet line of sources. Nothing that has an affordance of its own is repeated here. The annotated terms are marked in the description, where a tap opens the same explanation and the same link. The people are chips on the slide, and the chip is how this application gives a person's context everywhere — the layer briefly took the people for itself, which made the one slide with more to say the one slide where a person could not be opened. What the layer does do with people is emphasize them where the report names them: `parseBackgroundBlocks` runs the same matcher the description runs, against the whole ego network rather than the event's own cast, and marks every name the network knows as a `.person-mention`. It is emphasis and not an affordance — the chip a screen up is where the person opens — and a report ranging over a decade names people the event itself never did.
