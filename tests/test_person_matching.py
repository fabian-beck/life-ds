"""Tests for the port of the interface's person-name matcher.

``scripts/utils/person_matching.py`` ports ``getRelevantPeople`` from
``src/utils/story/personMatching.js``, and ``scripts/utils/event_depth.py``
selects deep events with it, so the port has to accept and reject the pairs
the interface does.
"""
...
from utils.person_matching import (  # noqa: E402
...
class InterfacePortTests(unittest.TestCase):
    def test_a_maiden_name_parenthetical_still_matches(self) -> None:
        self.assertTrue(
            interface_would_match("Ethel Sara Turing", "Ethel Sara Turing (née Stoney)")
        )

    def test_containment_carries_a_short_name(self) -> None:
        self.assertTrue(interface_would_match("John Quincy", "John Quincy Adams"))

    def test_diacritics_defeat_the_matcher(self) -> None:
        self.assertFalse(interface_would_match("Leó Szilárd", "Leo Szilard"))

    def test_eszett_defeats_the_matcher(self) -> None:
        self.assertFalse(
            interface_would_match("Marga von Hößlin Planck", "Marga von Hösslin")
        )

    def test_different_regnal_numbers_never_match(self) -> None:
        self.assertFalse(interface_would_match("Henry II", "Henry V"))
...
class ResolutionTests(unittest.TestCase):
    ...
    def test_a_suffix_breaks_the_tie_without_refusing_the_match(self) -> None:
        ...
        self.assertTrue(interface_would_match("Christian Bohr", "Christian Bohr Jr."))
